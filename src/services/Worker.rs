...
pub struct SubscriptionManager {
  ...
  pub events: collections::BTreeMap<Uuid, Option<crate::default_subscription::ResponseData>>,
...
impl SubscriptionManager {
  ...
  /// Check if a given UUID has an event ready Some(true)) or is still pending Some(false), 
  /// else None if UUID is unknown
  pub fn is_ready(
    &self, 
    id: &Uuid
  ) -> Option<bool> {
